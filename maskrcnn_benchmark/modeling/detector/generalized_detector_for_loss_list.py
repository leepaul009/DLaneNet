...
class LaneDetector_for_loss_list(nn.Module):
    def __init__(self, cfg):
        super(LaneDetector_for_loss_list, self).__init__()
        self.cfg = cfg
    ...
    def loss_evaluator(self, result, instances, targets, im_ids):
        ...
        #print("total_postive_num_batch size:{},  {}".format( total_postive_num_batch.size(), total_postive_num_batch ))
        ...
        ######################################################################## DIY BEG
        # print("fg_predict: {}, fpmask: {}".format( fg_predict.size(), fpmask.size() ))
        ...
        # print("total_cross_pos_batch size: {},  {}".format( total_cross_pos_batch.shape, total_cross_pos_batch ))
        ######################################################################## DIY END
        ...
        for i in range(batch_size):
            ...
            # upper line(5pts)
            curve_weighted_mask[mid_grid1 : mid_grid2, mid_pt+5 : mid_pt+10] = 15 # 10 mid->up ###!!! OPT-01 10->15
            # lower line(5pts)
        ...
        curve_weighted_mask[..., self.LANE_POINTS_NUM_DOWN] = 55 #50 #10 ###!!! OPT-01 50->55
        ...
        #if self.cfg.DEBUG:
        #    print(point_num_per_gt_anchor.size())
        #    print("loc_smooth_l1_loss loss: {}".format( loc_smooth_l1_loss.size() ))
        #    print("loc_loss_batch loss: {} {}".format( loc_loss_batch.shape, loc_loss_batch ))
        ######################################################################## DIY END
        ...
        for instance in instances:
            ...
            # print("sisc_loss_batch size: {}  {} ".format( sisc_loss_batch.shape, sisc_loss_batch ))
            ...
            # print("disc_loss_batch size: {}  {} ".format( disc_loss_batch.shape, disc_loss_batch ))

        total_ins = ALPHA * sisc_loss + BETA * disc_loss

        ############################################################# calculate batch loss
        total_ins_loss_batch = ALPHA * sisc_loss_batch + BETA * disc_loss_batch
        # print("total_ins_loss_batch size: {}  {} ".format( total_ins_loss_batch.shape, total_ins_loss_batch ))

        overall_loss_batch = total_cross_pos_batch + total_cross_neg_batch + loc_loss_batch + total_ins_loss_batch
        # print("overall_loss_batch size: {}  {} ".format( overall_loss_batch.shape, overall_loss_batch ))

        #for i, loss_per_batch in enumerate(overall_loss_batch):
        #    # node = hard_sampling.sampling_node(loss = hard_loss.cpu().data, data = data_list[i], previous_node = None, next_node = None)
        #    node = hard_sampling.sampling_node(loss = loss_per_batch, data = im_ids[i], previous_node = None, next_node = None)
        #    self.hard_sampler.insert(node)
