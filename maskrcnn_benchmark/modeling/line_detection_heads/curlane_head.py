###
# New implementation of fusion and head modules
import torch
...
class LDHeadModule(nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        lane_up_pts_num   = cfg.INPUT.ANNOTATION.POINTS_PER_LANE + 1 # 81(num+pts)
        lane_down_pts_num = cfg.INPUT.ANNOTATION.POINTS_PER_LANE # 80
        self.lane_up_pts_num = lane_up_pts_num
        self.lane_down_pts_num = lane_down_pts_num
        self.num_classes = num_classes
        BatchNorm = nn.BatchNorm2d

        # line in upper image(far)
        self.upper_lane_pred = nn.Sequential(
            nn.Conv2d(in_channels, base_channel, kernel_size=1, bias=False),
            BatchNorm(base_channel),
            nn.ReLU(),
            nn.Conv2d(base_channel, lane_up_pts_num, kernel_size=1, stride=1) # 81
        )
        # line in lower image(near)
        self.lower_lane_pred = nn.Sequential(
            nn.Conv2d(in_channels, base_channel, kernel_size=1, bias=False),
            BatchNorm(base_channel),
            nn.ReLU(),
            nn.Conv2d(base_channel, lane_down_pts_num, kernel_size=1, stride=1) # 80
        )
    ...
    # Fusion Pattern: 122 022
    def forward(self, input):
        predict_up   = self.upper_lane_pred(input).permute((0, 2, 3, 1)) # 81
        predict_down = self.lower_lane_pred(input).permute((0, 2, 3, 1)) # 80
        predict_cls  = self.cls_logits(input).permute((0, 2, 3, 1)).contiguous()

        predict_loc = torch.cat([predict_down, predict_up], -1).contiguous()
        predict_loc = predict_loc.view(predict_loc.shape[0], 
                                       -1, 
                                       self.lane_up_pts_num + self.lane_down_pts_num)
        predict_cls = predict_cls.view(predict_cls.shape[0], 
                                       -1, 
                                       self.num_classes)
        result = dict(
...
# Grid Fusion Network
def build_grid_fusion_nets(cfg, in_channels): 
    return GFNModule(cfg, in_channels)

# Lane Detection Head
def build_lane_detect_heads(cfg, in_channels):
