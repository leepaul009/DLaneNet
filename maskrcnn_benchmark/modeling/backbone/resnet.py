...
class ResNet(nn.Module):
    def __init__(self, cfg):
        ...
        stem_module = _STEM_MODULES[cfg.MODEL.RESNETS.STEM_FUNC] # ex. StemWithFixedBatchNorm
        stage_specs = _STAGE_SPECS[cfg.MODEL.BACKBONE.CONV_BODY] # ex. ResNet50FPNStagesTo5
        ...
        for stage_spec in stage_specs:
            ...
            stage_with_dcn = cfg.MODEL.RESNETS.STAGE_WITH_DCN[stage_spec.index -1] # ex. F F F F
            module = _make_stage(
                ...
                num_groups,
                cfg.MODEL.RESNETS.STRIDE_IN_1X1, # ex. T
                first_stride=int(stage_spec.index > 1) + 1, # 1, 2, 2, 2
                dcn_config={
...
# ex. res50:
# =>1,   (  64, 64, 256,s=1) => ( 256, 64, 256,s=1)x2
# =>1/2, ( 256,128, 512,s=2) => ( 512,128, 512,s=1)x3
# =>1/4, ( 512,256,1024,s=2) => (1024,256,1024,s=1)x5
# =>1/8, (1024,512,2048,s=2) => (2048,512,2048,s=1)x2
def _make_stage(
    transformation_module,
    in_channels,
    bottleneck_channels,
    ...
    blocks = []
    stride = first_stride
    for _ in range(block_count): # ex. 3, 4, 6, 3
        blocks.append(
            transformation_module(   # ex. BottleneckWithFixedBatchNorm
                in_channels,         # ex. 64 256 512 1024
                bottleneck_channels, # ex. 64, 128, 256, 512
                out_channels,        # ex. 256, 512, 1024, 2048
                num_groups,          # ex. 1
                stride_in_1x1,       # ex. T
                stride,              # ex. 1, 2, 2, 2
                dilation=dilation,   # ex. 1
                dcn_config=dcn_config
            )
        )
        stride = 1
