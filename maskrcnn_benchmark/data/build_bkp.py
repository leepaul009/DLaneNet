...
def make_data_sampler(dataset, shuffle, distributed):
    if distributed:
        #print("33333333333333 use samplers.DistributedSampler")
        return samplers.DistributedSampler(dataset, shuffle=shuffle)
    if shuffle:
        #print("44444444444444 use torch.utils.data.sampler.RandomSampler")
        sampler = torch.utils.data.sampler.RandomSampler(dataset)
    else:
        #print("55555555555555 use torch.utils.data.sampler.SequentialSampler")
...
def make_batch_data_sampler(
    ...
    if aspect_grouping:
        ...
        #print("1111111111 use torch.utils.data.sampler.BatchSampler")
        batch_sampler = torch.utils.data.sampler.BatchSampler(
            sampler, images_per_batch, drop_last=False
        )
    
    if num_iters is not None:
        #print("2222222222222 use samplers.IterationBasedBatchSampler")
        batch_sampler = samplers.IterationBasedBatchSampler(
            batch_sampler, num_iters, start_iter
        )
        #batch_sampler = samplers.IterationBasedBatchSampler(
        #    dataset, batch_sampler, num_iters, start_iter
        #)
