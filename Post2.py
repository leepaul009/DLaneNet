...
def create_groups_by_distance(lines, thresh=15):
    ...
    for i in range(nlines):
        for j in range(i+1, nlines):
            if i == j:
                ...
                dist, (udis, ldis), overlap = compute_dist_with_fit(lines[i], lines[j])
                dmat[i,j] = dist
                dmat[j,i] = dist
                uldmat[i,j,0], uldmat[j,i,0] = udis, udis
                uldmat[i,j,1], uldmat[j,i,1] = ldis, ldis
                omat[i,j], omat[j,i] = overlap, overlap
    ...
    while last_num > 0:
        ...
        while sid < nlines:
            ...
            pass_check = True
            for lid in groups[-1]:
                if (dmat[sid, lid] > thresh) or (uldmat[sid, lid, 0] > thresh/2. and omat[sid, lid] > 0.1):
...
###################### load input
rank = get_rank()
input_path = "exp/oct29/inference/aicom_lane_test/oct29_265h_r{}.pth".format(rank)
...
for pidx, (img_id, prediction) in tqdm( enumerate( predictions.items() ) ):
    ...
    # final_lines = drop_short_line(final_lines, thresh=200.) # (thresh => 1280x720)
