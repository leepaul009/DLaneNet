...
def draw_lines(x, y, image):
    ...
    for i, j in zip(x, y):
        ...
        for index in range(len(i)-1):
            ...
            image = cv2.circle(image, (int(i[index+1]), int(j[index+1])), 2, p.color[color_index], -1)
...
### 
# x, y: xs and ys of line
# point1: start point
# point2: close upper point
...
def sort_along_y_asc(x, y):
    ...
    for i, j in zip(x, y):
        ...
        ind = np.argsort(j, axis=0)
        out_x.append(np.take_along_axis(i, ind, axis=0).tolist())
        out_y.append(np.take_along_axis(j, ind, axis=0).tolist())
