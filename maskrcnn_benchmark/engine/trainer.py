...
def do_train(
    ...
    epoch_iters = int( 79992 / cfg.SOLVER.IMS_PER_BATCH)

    for iteration, (images, targets, image_ids) in enumerate(data_loader, start_iter):
        iteration = iteration + 1
        arguments["iteration"] = iteration

        if cfg.DEBUG and iteration == (start_iter+3):
        ...
        loss_dict = model(images, targets, image_ids)
        ...
        if iteration % epoch_iters == 0:
            data_loader.sampler.set_epoch( int( iteration / epoch_iters)+1 )

        # validation  
        if data_loader_val is not None and test_period > 0 and (iteration % test_period == 0 or cfg.DEBUG):
            ...
            with torch.no_grad():
                ...
                for iteration_val, (images_val, targets_val, image_ids_val) in enumerate(tqdm(data_loader_val)):
                    images_val  = images_val.to(device)
                    targets_val = dict(gt_loc=targets_val['gt_loc'].to(device), 
                                       gt_cls=targets_val['gt_cls'].to(device),
                                       gt_ins=targets_val['gt_ins'].to(device) )
                    ...
                    if cfg.DEBUG and iteration_val == 10:
                        print("")
                        break
            synchronize()
            model.train()
