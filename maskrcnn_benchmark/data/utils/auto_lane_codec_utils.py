...
# [[(x,y)...]...], del pt with same y, line from img bottom to top
    ...
    for line in lane_object["Lines"]:
        ...
        for point_index in range(len(line)):
            if line[point_index]["x"] == "nan" or line[point_index]["y"] == "nan":
                continue
            if not line[point_index]["y"] in set_y: # del pt with same y
                set_y.append(line[point_index]["y"])
                one_line.append((float(line[point_index]["x"]) * scale_x,
                                 float(line[point_index]["y"]) * scale_y))
        if len(one_line) >= 2:
            if one_line[0][1] < one_line[1][1]: # compare y between 1st and 2nd pt
                one_line = one_line[::-1] # line from img bottom to top
            gt_lane_list.append(one_line)
    return gt_lane_list

# list of Lane, np.array of Point
def trans_to_lane_with_type(gt_lanes_list):
    ...
    for lane in gt_lanes_list:
        ...
        if new_lane.size >= 2: # more than 2 pts
            prob = 1
            type = cnt # line id(from 1)
            c_x = 0
            c_y = 0
            start_pos = 0
            end_pos = 0
...
def get_lane_loc_list(all_anchor_distance, all_anchor_loc, h, w):
    ...
    gt_loc_list = list()
    gt_dist_list = list()
    gt_lid_list = list() # list of lane id
    lane_num = len(all_anchor_distance)
    for i in range(lane_num): # each lane
        ...
        for j in range(anchor_num):
            ...
            if cur_h == h and cur_w == w:
                ...
                gt_lid_list.append(i+1)
    return gt_loc_list, gt_dist_list, gt_lid_list
...
def gettopk_idx(gt_dist_list):
    ...
    top_idx = np.argsort(distance_list)[:1] # get smallest dist
...
def nms_with_pos(lane_set, thresh):
    ...
    for n in range(len(lane_sorted)):
        ...
        for t in range(n + 1, len(lane_sorted)):
            ...
            if dis <= thresh:
                selected[t] = True # reject t-th lane
    return lanes_result
