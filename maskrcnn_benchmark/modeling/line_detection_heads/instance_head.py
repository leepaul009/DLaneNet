...
class bottleneck_dilation(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        if in_channels < 4:
            temp_channels = in_channels
        self.conv1 = Conv2D_BatchNorm_Relu(in_channels, temp_channels, 1, 0, 1)
        self.conv2 = Conv2D_BatchNorm_Relu(temp_channels, temp_channels, 3, 1, 1) #, dilation=1) # why dilation?
        self.conv3 = nn.Conv2d(temp_channels, out_channels, 1, padding=0, stride=1, bias=True)
        #self.residual = Conv2D_BatchNorm_Relu(in_channels, out_channels, 1, 0, 1)
        self.dropout = nn.Dropout2d(p=0.1)
        self.prelu = nn.PReLU()
...
class Output(nn.Module):
    def __init__(self, in_size, out_size):
        ...
        # tmp_c1 = in_size//2 if in_size >= 2 else 2
        # tmp_c2 = in_size//4 if in_size >= 4 else 1
...
class hourglass_block(nn.Module):
    def __init__(self, cfg, in_channels, out_channels, acti = True, input_re=True):
        ...
        feature_size = cfg.INPUT.ANNOTATION.FEATURE_SIZE # 4
...
class instance_head(nn.Module):
    def __init__(self, cfg, in_channels, out_channels):
        ...
        self.ins_block1 = hourglass_block(
            cfg, 
            in_channels, 
            out_channels)
        self.ins_block1 = hourglass_block(
    ...
    def forward(self, features):
        # we can use:
        #   res [16 32] features[-2]
        #   res [32 64] features[-3]
        feature = features[-2] # [N 256 16 32]
        instance1, out = self.ins_block1(feature)
        instance2, out = self.ins_block2(out)
        instances = [instance1, instance2]
        return instances

def build_instance_head(cfg, in_channels):
    out_channels = cfg.MODEL.HG.INTERNAL_CHANNEL
    return instance_head(cfg, in_channels, out_channels)
